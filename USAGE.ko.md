...
시뮬레이션이 2D로 동작합니다(z는 0에 고정). 좌클릭 드래그 = 팬,
우클릭 드래그 = 3D 월드 틸트/회전, 휠/핀치 = 줌이며, 키보드는 팬 스킴
(화살표/WASD 팬, z/x 줌)을 사용합니다. `focusNode`/`panTo`/클릭 포커스는
평면 기준으로 동작하며 정면 뷰로 리셋됩니다.

밀집 그래프에서는 `enableNodeDrag={false}`도 함께 설정하세요 — 아니면
거의 모든 지점에서 포인터가 노드에 raycast되어 좌드래그가 팬 대신 노드를
잡아버립니다.
...
v1.4.4부터 회전은 화면 기준(트랙볼)입니다: 수평 드래그는 현재 뷰의 up 축,
수직 드래그는 right 축을 중심으로 회전하며, 피벗은 화면 중앙의 orbit
target입니다. 팬은 항상 화면 축을 따르고, 휠 줌은 target을 움직이지 않고
카메라만 다가가며, zoom-to-fit이 수평선(롤)을 리셋합니다.

드래그 팬 속도는 줌 적응형입니다(v1.4.5): 그래프 전체가 보일 땐 커서 정확
(1:1), 깊이 확대할수록 최대 3배까지 가속되어 큰 그래프도 답답하지 않게
이동합니다. `navigation.panSpeed`가 그 위에 곱해집니다.
